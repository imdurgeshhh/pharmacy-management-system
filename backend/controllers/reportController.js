...
exports.getDashboardStats = async (req, res) => {
    try {
        ...
        const todaySalesRes = await pool.query(
            'SELECT COALESCE(SUM(total_amount), 0) as total FROM SALES WHERE admin_id = $1 AND DATE(created_at) = CURRENT_DATE',
            [adminId]
        );
        const monthSalesRes = await pool.query(
            'SELECT COALESCE(SUM(total_amount), 0) as total FROM SALES WHERE admin_id = $1 AND extract(month from created_at) = extract(month from CURRENT_DATE)',
            [adminId]
        );
        ...
        const lowStockCountRes = await pool.query(
            'SELECT COUNT(*) FROM INVENTORY WHERE admin_id = $1 AND stock_qty < 20',
            [adminId]
        );
