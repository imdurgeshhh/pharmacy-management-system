...
const Login = () => {
    ...
    return (
        <main className="min-h-screen flex relative overflow-hidden bg-white">
                ...
                <div className="mx-auto w-full max-w-sm lg:w-96 animate-slide-up">
                    ...
                    <div className="flex items-center gap-3 mb-10 group cursor-default">
                        <div className="w-12 h-12 rounded-2xl bg-gradient-to-br from-[rgb(var(--color-primary))] to-[rgb(var(--color-primary-light))] flex items-center justify-center text-white shadow-xl shadow-[rgb(var(--color-primary))]/20 transition-transform duration-500 group-hover:rotate-12 group-hover:scale-110">
                            <Package size={24} strokeWidth={2.5} aria-hidden="true" />
                        </div>
                        <h2 className="text-3xl font-display font-extrabold tracking-tight text-[rgb(var(--text-heading))] text-balance">{APP_BRAND}</h2>
                    </div>
                    ...
                    <div className="card-glass p-8 relative">
                        ...
                        <h3 className="text-2xl font-bold font-sans tracking-tight mb-2 text-[rgb(var(--text-heading))] text-center sm:text-left text-balance">Welcome Back</h3>
                        <p className="text-sm text-[rgb(var(--text-body))] mb-8 text-center sm:text-left text-pretty">
                            Authenticate using Clerk to access your workspace.
                        </p>

                        <div className="space-y-4 relative z-10">
                            <Show when="signed-out">
                                <SignInButton mode="modal" forceRedirectUrl="/">
                                    <button className="w-full flex justify-center items-center gap-2 btn-primary shadow-[rgb(var(--color-primary))]/30 py-3.5">
                                        Sign In with Clerk <ArrowRight size={18} aria-hidden="true" />
                                    </button>
                                </SignInButton>

                                <SignUpButton mode="modal" forceRedirectUrl="/">
                                    <button className="w-full flex justify-center items-center gap-2 border border-[rgb(var(--border-subtle))] bg-white hover:bg-slate-50 text-[rgb(var(--text-body))] font-medium py-3.5 rounded-xl transition-[background-color,border-color,transform] duration-200 active:scale-[0.96] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[rgb(var(--color-primary))]">
                                        Create New Account
                                    </button>
                                </SignUpButton>
                            </Show>
